...
class aa_publisher(Node):
    def __init__(self):
        ...
        self.calib = {'color': {'fx': 439.09984732, 'fy': 440.97311686,
                                      'ppx': 428.10459275, 'ppy': 236.80079264, 
    ...
    def _execute_approach(self, trajectory_list, demo_image_list):
        ...
        trajectory_list = trajectory_list[40:] # skip first 40 to avoid jitter
        ...
        cv2.waitKey(16)
        print(f"Demo images:", demo_image_list)



    def _execute_move(self, trajectory_list, demo_image_list):
        self.get_logger().info(f"Move to target pose...")
        trajectory_list = trajectory_list[40:] # skip first 20 to avoid jitter
        if self.current_pose is not None:
            self.current_pose[0] = init_id
            # self.action_publish(self.current_pose)
        ...
        sample_num = len(trajectory_list)
        for i in range (len(trajectory_list)):
            if trajectory_list[i][3] <= 0.31:
                trajectory_list[i][3] = 0.31
            target_pose = trajectory_list[i].copy()
            target_pose[-1] = 0.02
            target_pose.insert(0, i+1)
            self.action_publish(target_pose)
    ...
    def _execute_grasp(self, trajectory_list, demo_image_list):
        ...
        self.action_publish(rotated_pose_array)
        self.field_img_color, self.field_img_depth = self.wait_feedback(-1)
        # Update current pose
    ...
    def _execute_insert(self, trajectory_list, demo_image_list):
        ...
        # Get the pose with gripper rotated to horizon
        print("Rotate camera to horizon")
        # pose_7element = np.array(self.current_pose.copy()[1:-1])  #np.array([x,y,z,rx,ry,rz,rw])
        # print(pose_7element)
        # initial_pose_7element_horizon = rotGripper2Horizon(pose_7element)
        # rotated_pose = [target_id] + initial_pose_7element_horizon.tolist() + [self.current_pose[-1]]
        # rotated_pose_array = np.array(rotated_pose)
        ...
        rotated_pose_array[3] += 0.02  # lift up a bit to avoid hitting the table
        rotated_pose_array[1] += 0.05 # 後退以看到洞口
        ...
        # Get the object's current location with SAM (Camera frame)
        xyz_c = get_xy_xyz_for_insert(demo_image_list, self.field_img_color, self.field_img_depth, self.calib)

        return

        xyz_w_to_cam_pose = self.xyz_w_to_cam_pose(xyz_c, rotated_pose_array)
        ...
        # self.current_pose[0] = init_id
        # self.action_publish(self.current_pose)
        # self.action_publish(pre_insert_pose)
        # self.field_img_color, self.field_img_depth = self.wait_feedback(target_id)
        # # Update current pose
        # self.current_pose = pre_insert_pose.copy()
        # # Show feedback image
        # cv2.imshow("field_img_color", self.field_img_color)
        # cv2.waitKey(0)

        # # Publish insert pose
        # insert_pose_temp = [target_id] + xyz_w.tolist() + [self.current_pose[-1]]
        # insert_pose = np.array(insert_pose_temp)
        # insert_pose[3] += 0.02  # avoid hitting the table
        # self.current_pose[0] = init_id
        # self.action_publish(self.current_pose)
        # self.action_publish(insert_pose)
        # self.field_img_color, self.field_img_depth = self.wait_feedback(target_id)
        # cv2.imshow("field_img_color", self.field_img_color)
        # cv2.waitKey(0)
        # self.current_pose = insert_pose.copy()
    def xyz_w_to_cam_pose(self,xyz_c,current_pose):
    ...
    def insert(self,holePos_A_C,lastPos_A_C):
        ...
        holePosTouch[0] = 1.0
        holePosTouch[-1] = 0.0290
        self.action_publish(holePosTouch.tolist())
        holePosFinal[0] = 2.0
        holePosFinal[-1] = 0.0290
    ...
    def rotCam2Horizon(self,T_a_ci):
        ...
        T_a_ci[4:8] = R.from_euler('xyz', [180.0, 0.0, rot[2]], degrees=True).as_quat()
        T_a_g = self.c2g(T_a_ci.copy())
        T_a_p = self.g2p(T_a_g.copy(),0.09)
        ...
        for pt in data.get("primitive_tasks", []):
            ...
            for i in range(len(atomic_actions_list)):
                ...
                if next_act is not None:
                    params = next_act.get("parameters", {})
                    next_demo_image_path_list = params.get("image_path", [])
...
def main():
    ...
    workflow_path = "exe_data_dummy/task_workflow.json"
    # Initialize ROS 2
    rclpy.init()
