...
from xyz_pc_f_fs_for_fixed_hole import get_xy_xyz_for_grasp, get_xy_xyz_for_insert, c2e, rotGripper2Horizon 
...
class aa_publisher(Node):
    def __init__(self):
        ...
        self.calib = {'color': {'fx': 437.3045820512652, 'fy': 437.2467361094098,
                                      'ppx': 422.3146630863898, 'ppy': 242.3790009095984, 
    ...
    def _execute_approach(self, trajectory_list, demo_image_list):
        ...
        trajectory_list = trajectory_list[30:] # skip first 40 to avoid jitter
    ...
    def _execute_move(self, trajectory_list, demo_image_list):
        ...
        trajectory_list = trajectory_list[30:] # skip first 20 to avoid jitter
        sample_num = len(trajectory_list)
        for i in range (len(trajectory_list)):
            if trajectory_list[i][2] <= 0.32:
                trajectory_list[i][2] = 0.32
            target_pose = trajectory_list[i].copy()
            target_pose[-1] = 0.029
            target_pose.insert(0, i+1)
            self.action_publish(target_pose)
    ...
    def _execute_insert(self, trajectory_list, demo_image_list):
        ...
        # Get the object's current location with SAM (Camera frame)
        # quad_name = get_xy_xyz_for_insert(demo_image_list, self.field_img_color, self.field_img_depth, self.calib)
        quad_name = 'bottom_left'  # for fixed hole test
        print("Target quadrant for insertion:", quad_name)
        # Publish pre-insert pose
        pre_insert_pose = fixedhole_A_C[quad_name]
...
def main():
    ...
    "/home/lab606/ICA_Lab_UMI/UMI_exec/task_workflow.json"
